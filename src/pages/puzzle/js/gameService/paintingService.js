...
const getLevelPaintingsCollection = () =>
  [paintings1, paintings2, paintings3, paintings4, paintings5, paintings6][store.level - 1];
...
const getRandomPainting = () => {
  const collectionsArray = [paintings1, paintings2, paintings3, paintings4, paintings5, paintings6];
...
const getPaintingCutSrc = () =>
  `${paintingUrl}${store.playUserWords ? getRandomPainting().cutSrc : getRoundPainting().cutSrc}`;
const getPaintingImageSrc = () =>
  `${paintingUrl}${
    store.playUserWords ? getRandomPainting().imageSrc : getRoundPainting().imageSrc
  }`;
...
const showBackgroundPic = () => {
  painting.style.zIndex = 1;
};
const hideBackgroundPic = () => {
  painting.style.zIndex = -2000;
};
