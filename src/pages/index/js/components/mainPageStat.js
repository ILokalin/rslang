...
export const renderShortTermStat = async (settings, shortTermStat) => {
  ...
  if (shortTermStat && shortTermStat.date === today) {
    ...
    document.querySelector('.statistics__repeated-words').innerText =
      shortTermStat.totalCards - shortTermStat.newWords;
    ...
    const angle = Math.round(
      (((180 * (shortTermStat.totalCards || 0)) / settings.cardsPerDay) * 100) / 100,
    );
    document
      .querySelector('.sc-percentage')
      .setAttribute('style', `transform: rotate(${angle > 180 ? 180 : angle}deg);`);
  } else {
  ...
  if (settings) {
    ...
    document.querySelector('.plan_repeat').innerText =
      settings.cardsPerDay - settings.newCardsPerDay;
