export const messages = [
  { time: 3000, text: 'Мы подбираем для вас нужные слова' },
  { time: 1000, text: 'Бережно переводим каждое слово' },
  { time: 3000, text: 'Художники уже рисуют карточки' },
  { time: 1500, text: 'Кастинг актеров для озвучивания слов завершен' },
  { time: 2000, text: 'Запись перевода ведется в лучших студиях' },
  { time: 2800, text: 'Курьер с карточками уже выехал' },
  { time: 2000, text: 'Приготовьтесь! Мы уже начинаем...' },
  { time: 2000, text: 'Это сложный процесс - можно было успеть заказать пиццу' },
  { time: 2000, text: 'Не против, если я расскажу чуть больше о процессе подготовки?..' },
];

export const textOfFacts = [
  'Каждые два часа в английский словарь добавляется новое слово. За год это около 4000 слов!',
  'Первый словарь английского языка написан в 1755 году.',
  'Самые старые английские слова, которые употребляются и сейчас: town, I, we, two, three.',
  'На английском говорит 1 миллиард человек, а это каждый 7-й человек на планете.',
  'Английский - официальный язык неба. Совершая международные перелеты, пилоты общаются на английском вне зависимости от своего родного языка и национальности.',
  'Из всех английских слов чаще всего неправильно произносят "pronunciation" - "произношение"!',
  'Символ @ в английском называется "the at sign" или "the at symbol".',
  'У символа # есть несколько названий в английском: hash, pound sign, number sign.',
  'Точка над буквами i и j называются superscript dot.',
  'В английском языке в каждом слоге должен быть гласный звук, но не в каждом слоге есть согласный.',
  'Некоторые английские слова используются только во множественном числе: glasses, scissors, trousers, jeans, pyjamas.',
  'Английский идет на втором месте после мандаринского диалекта китайского по числу говорящих на нем. Это официальный язык 67 стран.',
  'Большинство жителей США в разговоре пользуются американским английским. В стране 24 диалекта английского.',
  '"Go!" - это самое короткое грамматически верное английское предложение.',
  'Символ & когда-то был полноценной буквой английского алфавита.',
  'Самые употребляемые слова в английском: местоимения I и You, прилагательное - good, существительное - time.',
  'Шекспир изобрел много английских слов, среди которых: birth place, blushing, torture.',
  'Около 80% информации, которая хранится на компьютерах в мире, на английском языке.',
  'Слово goodbye происходит от фразы "God be with you." ("Да пребудет с тобой Господь.")',
  'Произношение слова queue не изменится, даже если исключить из него все буквы, кроме первой.',
  'Есть 2 особенные буквы в английском алфавите: E - самая частая, и английская Q - самая редкая.',
  'В английском языке не было системы пунктуации вплоть до введения печати в XV веке. До этого момента знаки препинания практически отсутствовали.',
];
